...
# Drop features that are redundant, volatile, or non-actionable for lecturers
columns_to_drop = ['school', 'nursery', 'romantic', 'reason', 'guardian', 'famsup', 'higher', 'paid']
df_filtered = df.drop(columns=columns_to_drop)
print(f"✂️ Filtered out {len(columns_to_drop)} unnecessary noise columns. Retained: {df_filtered.shape[1]} attributes.")
